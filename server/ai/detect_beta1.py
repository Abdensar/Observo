...
def main():
    ...
    print(f"Using RTSP URL: {VIDEO_PATH}")
    ...
    try:
        while True:
            ...
            if monitor.enabled_features['protected_zone'] and monitor.protected_zone:
                exterior = np.array(monitor.protected_zone.exterior.coords[:-1], dtype=np.int32)
                cv2.polylines(frame, [exterior], True, (0, 255, 255), 2)
                cv2.putText(frame, "Protected Zone", 
                           tuple(map(int, monitor.protected_zone.exterior.coords[0][:2])),
            ...
            for alert_type, message in alerts:
                ...
                img_path = monitor.save_alert(frame, alert_type, message)
                
                color = monitor.alert_colors.get(alert_type, (0, 0, 255))
                cv2.putText(frame, f"ALERT: {message}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
                cv2.putText(frame, f"Screenshot saved", (10, 60),
...
# Initialize the monitor globally so it can be accessed in the video_feed function
model = YOLO(MODEL_PATH)
monitor = SecurityMonitor(model)
...
def video_feed():
    def generate():
        cap = monitor.get_video_capture(VIDEO_PATH)
        if not cap.isOpened():
            logger.error(f"Failed to open RTSP stream: {VIDEO_PATH}")
            yield b''
            return
        ...
        while True:
            ...
            # Apply detection logic and process the frame
            results = monitor.model(frame, classes=[0, 2], verbose=False)
            alerts = monitor.update_detections(frame, results)
...
if __name__ == "__main__":
    main()
    app.run(host="0.0.0.0", port=5002)  # Bind to 0.0.0.0 for external access

    # Example usage of RTSP to HLS conversion
    rtsp_url = "rtsp://localhost:8554/test2"
    output_dir = "hls_output"
    os.makedirs(output_dir, exist_ok=True)
    hls_url = convert_rtsp_to_hls(rtsp_url, output_dir)
    if hls_url:
