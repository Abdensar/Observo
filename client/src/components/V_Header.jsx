...
const Header = ({CheckVis}) => {
  ...
  return (
    ...
    {Visitor?( <section className='fixed bg-gray-950 w-full h-14 flex px-8 py-2 '>
        ...
        <Link  className='flex h-full align-middle' onClick={()=>handleScroll('hero')}>
        ...
        <section className='self-center mx-4  w-full  p-2 text-center md:space-x-20 sm:space-x-10 text-xl italic font-semibold text-white  flex justify-center'>
            <span className='hover:text-gray-400 duration-500 cursor-pointer' onClick={()=>handleScroll('hero')}>Home</span>
            <span className='hover:text-gray-400 duration-500 cursor-pointer' onClick={()=>handleScroll('aboutUs')}>About Us</span>
